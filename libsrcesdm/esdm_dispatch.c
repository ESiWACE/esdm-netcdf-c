...
int ESDM_create(const char *path, int cmode, size_t initialsz, int basepe, size_t *chunksizehintp, void *parameters, const struct NC_Dispatch *table, int ncid) {
  DEBUG_ENTER("%s\n", path);
  ...
  NC* nc;
  int ret = NC_check_id(ncid,&nc);
  if(ret != NC_NOERR) {
    ERROR("Cannot retrieve NC ID\n");
  }
  nc->dispatchdata = e;
...
int ESDM_open(const char *path, int cmode, int basepe, size_t *chunksizehintp, void *parameters, const struct NC_Dispatch *table, int ncid) {
  const char *realpath = path;
  DEBUG_ENTER("%s\n", path);
...
int ESDM_get_vars(int ncid, int varid, const size_t *startp, const size_t *countp, const ptrdiff_t *stridep, void *data, nc_type mem_nc_type) {
  DEBUG_ENTER("%d %d\n", ncid, varid);
  int ret_NC = NC_NOERR;
